...
export const Cart: React.FC = () => {
    ...
    const totalCount = items.reduce((sum: number, item: CartItemType) => {
        return (item.count) ? sum + item.count : sum
    }, 0)

    const myTotalPrice = items.reduce((sum: number, item: CartItemType) => {
        return (item.count) ? sum + item.price * item.count : sum
    }, 0)
        ...
        <div className="container container--cart">
            <div className="cart">
                ...
                <div className="content__items">
                    ...
                    {items.map(cart => <CartItem key={cart.id} cart={cart} />)}

                </div>
                <div className="cart__bottom">
                    <div className="cart__bottom-details">
                        <span> Всего пицц: <b>{totalCount} шт.</b> </span>
                        <span> Сумма заказа: <b>{myTotalPrice} ₽</b> </span>
                    </div>
